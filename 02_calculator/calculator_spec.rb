...
describe "#multiply" do
  ...
  it "multiplies two other numbers - with decimal numbers" do
    # one other test here, don't forget do end ;)
    expect(multiply(-3, 4.8)).to be_within(0.01).of(-14.4)
  end

  it "multiplies two other numbers, one of them being 0" do
    # one last test, with 0 in it
    expect(multiply(0,899)).to eq(0)
  end
end
...
describe "#power" do
  ...
  it "raises one number to the power of another number - with decimal number as exponent" do
      expect(power(4,0.5)).to eq(2)
  end
  it "raises one number to the power of another number - with 0 as exponent" do
      expect(power(-32,0)).to eq(1)
  end
  it "raises one number to the power of another number - with 0" do
      expect(power(0,44)).to eq(0)
  end
